...
bool ClientConnection::is_registered() const {return state == ClientState::SYNCING || state == ClientState::LIVE || state == ClientState::REGISTERED;}
...
void ClientConnection::mark_registered() { state = ClientState::REGISTERED;}
...
void ClientConnection::set_current_document(const DocumentID& id) {
    current_document = id;
}

void ClientConnection::clear_current_document() {
    current_document.reset();
}

bool ClientConnection::has_current_document() const {
    return current_document.has_value();
}

const DocumentID& ClientConnection::get_current_document() const {
    if (!current_document.has_value()) {
        throw std::runtime_error("Client has no current document.");
    }
    return current_document.value();
}